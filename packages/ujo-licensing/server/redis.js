...
async function addStore(userAddress, { name, id, LicenseOwnership, LicenseSale, LicenseInventory }) {
  ...
  await client.hsetAsync(
    'stores:data',
    id,
    JSON.stringify({ name, id, LicenseOwnership, LicenseSale, LicenseInventory }),
  );
}
...
async function clearAll() {
  await Promise.all([client.delAsync('metadata'), scanForEach('stores:*', async keys => client.delAsync(...keys))]);
}
